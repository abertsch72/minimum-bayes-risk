...
from src.recom_search.model.merge_strategy import merge_imp, merge_zip
from src.recom_search.model.util import pnum, render_name, run_inference_step


def step_bfs_rcb_any(
    tokenizer,
    start_seed: BeamNodeFull,
    hash: HashObject,
    heap,
    doc_input_ids,
    model,
    ...
    while (not pointer.finished) and step < expl_steps:
        ...
        top1_state = BeamNodeFull(
            hash=hash,
            prob=values[0],
            token_idx=indices[0],
            prev=[pointer.uid],
            prev_score=[math.log(values[0])],
        )
        ...
        if cur_len >= hash.ngram:
            ...
            for cand_par in retrieved:
                ...
                try:
                    (
                        one_match_path_token_ids,
                        cand_par_suffix_node_ids,
                    ) = cand_par.get_tokens_match_suffix(ngram)
                except ValueError:
            ...
            if flag_merge:
                if merge_method == "zip":
                    merge_zip(
                        ...
                        cand_par,
                        top1_state,
                        par_match_uids=cand_par_suffix_node_ids,
                    )
        ...
        if (
            flag_merge
            or top1_state.finished
            or (step < expl_steps and (not top1_state.finished))
        ):
            values = values[1:]
            indices = indices[1:]
        ...
        for v, i in zip(values, indices):
            ...
            # print(v)
            tmp_state = BeamNodeFull(
                hash=hash,
                prob=v,
                token_idx=i,
                prev=[pointer.uid],
                prev_score=[math.log(v)],
                master_node_uid=top1_state.uid,
            )
            if (
                tmp_state.finished or tmp_state.length >= max_len
            ):  # if this branch is a completed one, just put it in the outputs.
...
def bfs_rcb_any(
    ...
    if config_search["post"]:
        budget_expl = comp_budget - int(config_search["post_ratio"] * comp_budget)
    else:
    ...
    for prefix in dec_prefix:
        if last:
            ...
            init_seed = BeamNodeFull(
                hash=core_hash_obj, prob=1.0, token_idx=prefix, prev=[], prev_score=[]
            )
    ...
    while ncalls < budget_expl:
        ...
        if config_search["dfs_expand"]:
            ...
            expl_steps = 1
        completed_hyps, added_num_calls = step_bfs_rcb_any(
            tokenizer,
            seed,
            core_hash_obj,
            heap,
    ...
    while ncalls < comp_budget:
        ...
        empty_heap = (
            []
        )  # at this stage, we do not put new nodes in the heap, hence we only need an empty heap.
    ...
    for hypo in finished_hypos:
        ...
        # hypo.print_lattice()
    logging.info(
        f"Fist Stage: {num_comp_hypo_1_stage}\Second Stage: {len(finished_hypos)}"
    )
