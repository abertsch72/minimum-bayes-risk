...
def step_bfs(
    ...
    while (not pointer.finished) and step < expl_steps:
        ...
        token_txt = (
            tokenizer.decode(indices[0]).strip().lower()
        )  # token of the decoded word
        ...
        for v, i in zip(values, indices):
            ...
            tmp_state = BeamNodeEz(
                prob=v, token_idx=i, prev=[pointer], prev_score=[math.log(v)]
            )
            if (
...
def bfs(
    ...
    for prefix in dec_prefix:
        if last:
            init_seed = BeamNodeEz(
                prob=1.0, token_idx=prefix, prev=[last], prev_score=[0]
            )
        else:
            init_seed = BeamNodeEz(prob=1.0, token_idx=prefix, prev=[], prev_score=[])
            last = init_seed
    ...
    while ncalls < budget_expl:
        ...
        completed_hyps, added_num_calls = step_bfs(
            tokenizer,
            seed,
            heap,
            doc_input_ids,
