...
class DeployHeu:
    ...
    def run(self, cur_node, prev_len, prob_distb):
        ...
        seq_score = (
            self.heuristic_seq_score(cur_node) * self.heu_config["heu_seq_score"]
        )
        len_rwd = (
            self.heuristic_seq_score_len_rwd(prev_len)
            * self.heu_config["heu_seq_score_len_rwd"]
        )
        ent = self.heuristic_ent(prob_distb) * self.heu_config["heu_ent"]
        pos_bias = self.heuristic_position(prev_len) * self.heu_config["heu_pos"]

        good_word_reward = self.heuristic_token(cur_node) * self.heu_config["heu_word"]
