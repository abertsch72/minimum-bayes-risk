...
def baseline_iterative_recomb(
    candidates: List[BeamNode], param_sim_function, beam_size
):
        ...
        for nx_cand in next_candidate:
            ...
            while abs(len_cand - pointer.length) < len_diff:
                flag = similarity_heuristic(
                    candidate.all_token_idx,
                    pointer.all_token_idx,
                    ngram_suffix,
                    len_diff,
                )
                if not flag:
...
def baseline_recomb_sample(
    ...
    while True:
        ...
        if feat_key in gen_nodes:
            ...
            tmp_node = BeamNodeEz(
                prob=next_tok_prob,
                token_idx=next_tok_idx,
                prev=[hypo],
                prev_score=[math.log(next_tok_prob)],
            )
...
def recomb_baseline(
    ...
    for t in range(max_len):
        ...
        for hypo in hypos:
            ...
            for idx, v, i in zip(range(beam_size), values, indices):
                tmp_state = BeamNodeEz(
                    prob=v, token_idx=i, prev=[hypo], prev_score=[math.log(v)]
                )
        ...
        if avg_score == -1:
            ...
            sorted_candidates = sorted(
                candidates,
                key=lambda x: x.get_score_sum() / len(x) ** avg_score,
                reverse=True,
            )
    ...
    # sort finished
    if avg_score == -1:
        finished = sorted(finished, key=lambda x: x.get_score_sum(), reverse=True)
    else:
        finished = sorted(
            finished,
            key=lambda x: x.get_score_sum() / len(x) ** avg_score,
