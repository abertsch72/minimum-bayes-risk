...
def run_bs_recombination(
    args, model, input_doc, dec_prefix, param_sim_function, adjust=True
):
    ...
    if adjust:
        adj_batch_size = adjust_batch_size(cur_max_len, args.task, args.dataset)
    else:
        adj_batch_size = args.beam_size
    output = recomb_baseline(
        doc_input_ids=input_ids,
        dec_prefix=dec_prefix,
        param_sim_function=param_sim_function,
        model=model,
        debug=False,
        beam_size=adj_batch_size,
        max_len=cur_max_len,
...
def run_recom_sample(
    args, model, input_doc, dec_prefix, param_sim_function
) -> SearchModelOutput:
    input_ids = tokenizer(input_doc, return_tensors="pt").input_ids.to(args.device)
    if args.max_len == -1:
        cur_max_len = input_ids.squeeze().size()[0] * 2
...
def run_bfs_recombination(
    args, model, tokenizer, inp, dec_prefix, param_sim_function, config_search
) -> SearchModelOutput:
    config_heu = {
    ...
    input_ids = tokenizer(
        inp, return_tensors="pt", padding=True, truncation=True, max_length=1024
...
def run_model(args, tokenizer, model, dataset, dec_prefix, wt_dir):
    ...
    for idx, example in enumerate(tqdm(dataset)):
        ...
        if args.task.startswith("mt"):
            ...
            raise NotImplementedError(
                "for customized dataset, use custom as the name of dataset and document|ref|uid for fields"
            )
