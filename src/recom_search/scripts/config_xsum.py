...
config_recom_sample = [
    " -model recom_sample -top_p 0.8 ",
    " -model recom_sample -top_p 0.9 ",
]
config_astar_base = [
    "-model astar_base -avg_score -1  -dfs_expand ",
    "-model astar_base -avg_score -1  -post -post_ratio 0.3 ",
]
baselines = (
    config_dbs
    + config_bs
...
for b in common_a_star:
    all_commands.append(
        cmd_base + f" -model astar -merge imp -device cuda:{cuda_range[i % 4]}  " + b
    )
    i += 1
    all_commands.append(
        cmd_base + f" -model astar -merge zip -device cuda:{cuda_range[i % 4]}  " + b
    )
    i += 1
    all_commands.append(
        cmd_base_low
        + f" -model astar -merge zip -device cuda:{cuda_range[i % 4]}  "
        + b
    )
    i += 1
