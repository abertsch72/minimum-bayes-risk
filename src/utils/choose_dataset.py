...
def get_dataset(
    ...
    elif dataset.name == DatasetArgs.SupportedDataset.aqua.name:
        full_data = full_data.map(
            lambda ex: {
                "input": (ex['question'], "(" + " (".join(opt.replace(")", ") ", 1) for opt in ex['options']).lower()),
                "output": (ex['correct'], ex['rationale'])
            },
            remove_columns=['question', 'options', 'correct', 'rationale']
        )
    elif dataset.name == DatasetArgs.SupportedDataset.gsm8k.name:
        full_data = full_data.rename_column("question", "input")
        full_data = full_data.rename_column("answer", "output")
