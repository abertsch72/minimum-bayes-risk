...
count = 0
models_by_dataset = {"cnndm": ["facebook/bart-large-cnn", "ainize/bart-base-cnn"], "samsum": ["lidiya/bart-base-samsum", "linydub/bart-large-samsum", "Chikashi/t5-small-finetuned-cnndm", "navjordj/t5-base-cnndm", "navjordj/t5-large-cnndm"]}
...
for dataset in ["cnndm", "samsum"]:
    ...
    for model in models_by_dataset[dataset]:
        for gen_strat in [Args.ListGenArgs.ModelSamplingArgs(temp=0.1), Args.ListGenArgs.ModelSamplingArgs(temp=0.7), Args.ListGenArgs.ModelSamplingArgs(top_p=0.3), Args.ListGenArgs.ModelSamplingArgs(top_p=0.5), Args.ListGenArgs.ModelSamplingArgs(temp=0.5)]:
            for rerank in ['rouge1', 'rouge2', 'rouge3', 'rouge4', 'rouge5', 'rouge6', 'bertscore', 'bartscore']:
