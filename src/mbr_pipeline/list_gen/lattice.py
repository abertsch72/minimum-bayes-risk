...
class Lattice(object):
    def __init__(self, node_dict, edge_dict):
        ...
        for (
            eos
        ) in self.eos_list:  # validate that eos nodes don't have any outgoing edges
    ...
    def _extract_length_dict(self, all_node_length_dict):
        ...
        for eos in self.eos_list:
            for length, (length_lprob, length_count) in all_node_length_dict[
                eos
            ].items():
                if length in length_dict:
                    old_lprob, old_count = length_dict[length]
                    length_dict[length] = (
                        np.logaddexp(old_lprob, length_lprob),
        ...
        while len(node_bag) > 0:
            ...
            for candidate_node in self.nodes.keys() - visited:
                all_parents_done = all(
                    parent in visited for parent in self.reverse_edges[candidate_node]
                )
                if all_parents_done:
                ...
                not_done_parents = {
                    parent
                    for parent in self.reverse_edges[candidate_node]
            ...
            if curr_node in visited:
                continue
            print(
                f"{curr_node}: {sum(i for (_, i) in node_length_dict[curr_node].values())}"
            )
            visited.add(curr_node)
            for child_node, edge_score in self.edges[curr_node].items():
                child_node_length_dict = node_length_dict[child_node]
                for length, (length_score, length_count) in node_length_dict[
                    curr_node
                ].items():
    ...
    def get_length_dict(self):
        ...
        all_node_length_dict[self.sos] = {0: (0.0, 1)}
            ...
            for parent_node, edge_lprob in self.reverse_edges[node].items():
                ...
                for parent_length, (
                    ...
                    if new_length in curr_length_dict:
                        ...
                        new_entry = (
                            np.logaddexp(old_lprob, added_lprob),
    ...
    def _get_path_count_bounded_length(self, length_data, target_length, max_deviation):
        ...
        for length, (lprob, c) in length_data.items():
            if (
                length <= target_length + max_deviation
                and length >= target_length - max_deviation
            ):
                all_lprobs.append(lprob)
                total_count += c
    ...
    def _extract_word_dict(self, all_node_word_dict):
        ...
        for eos in self.eos_list:
            for word, (word_lprob, word_count) in all_node_word_dict[eos].items():
                ...
                word_dict[word] = (
                    np.logaddexp(old_lprob, word_lprob),
                    old_count + word_count,
                )
        return word_dict

    def get_1gram_dict(
        self, all_node_length_dict, target_length=None, allowed_deviation=0
    ):
        """
        ...
        def dfs_helper(node, length_to_here=0):
            ...
            for parent_node, edge_lprob in self.reverse_edges[node].items():
                parent_word_dict = dfs_helper(
                    parent_node, length_to_here=length_to_here + 1
                )
    ...
    def get_1gram_dict_count_aware(
        self, all_node_length_dict, target_length=None, allowed_deviation=0
    ):
        """
        ...
        def dfs_helper(node, length_to_here=0):
            ...
            if total_lprob != float("inf"):
                ...
                # count \in [max_count, ..., 2]
                for count in range(max_count, 1, -1):
                    curr_word_dict[(curr_word, count)] = curr_word_dict[
                        (curr_word, count - 1)
                    ]
                    other_count_scores.append(curr_word_dict[(curr_word, count - 1)][0])
                    other_count_paths += curr_word_dict[(curr_word, count - 1)][1]
                # want to subtract lprobs
                scaling = -np.ones(len(other_count_scores))
                scaling[0] = 1
        ...
        def dfs_helper(node, length_to_here=0):
            if node == self.sos:
                ...
                if length_to_here >= target_length + allowed_deviation:
                    ...
                    if True in [
                        (abs(length_to_here - visited_len) <= allowed_deviation)
                        for visited_len in visited[node]
                    ]:
                        return all_node_word_dict[node]
            ...
            for parent_node, edge_lprob in self.reverse_edges[node].items():
                ...
                old_lprob, old_count = curr_word_dict.get(
                    curr_bigram, (-float("inf"), 0)
                )
                if target_length is not None:
                    parent_lprob, parent_count = self._get_path_count_bounded_length(
                        all_node_length_dict[parent_node],
                        target_length - (length_to_here + 1),
                        allowed_deviation,
                    )
                else:
                    parent_lprob, parent_count = path_count_dict[parent_node]
                added_lprob = parent_lprob + edge_lprob
                curr_word_dict[curr_bigram] = (
                    np.logaddexp(old_lprob, added_lprob),
                    old_count + parent_count,
                )
    ...
    def get_1gram_dict_by_length(
        self, all_node_length_dict, target_length=0.0, allowed_deviation=float("inf")
    ):
        """
        all_node_word_dict = {
            node: {
        ...
        def dfs_helper(node):
            ...
            for parent_node, edge_lprob in self.reverse_edges[node].items():
                ...
                for (
                    parent_word,
                    parent_length_dict,
                ) in parent_word_dict_all_lengths.items():
        ...
        min_length, max_length = (
            target_length - allowed_deviation,
            target_length + allowed_deviation,
        )
        ...
        for eos in self.eos_list:  # sum reduction over nodes
            for eos_word, eos_length_dict in all_node_word_dict[eos].items():
                ...
                for eos_length, (
                    eos_word_lprob,
                    eos_word_count,
                ) in eos_length_dict.items():
                    if not (min_length <= eos_length <= max_length):
                        continue
                    # old_lprob, old_count = agg_length_dict.get(eos_length, (-float('inf'), 0))
                    old_prob, old_count = agg_length_dict.get(
                        eos_length, (-float("inf"), 0)
                    )
                    agg_length_dict[eos_length] = (
                        np.logaddexp(old_prob, eos_word_lprob),
    ...
    def _extract_top_gain_path(
        self, all_node_rouge_dict, min_length=0, max_length=float("inf")
    ...
    def _extract_topk_gain_paths(
        self, all_node_topk_rouge_dict, min_length=0, max_length=float("inf"), topk=1
    ):
        topk_eos = heapdict()
        for eos in self.eos_list:
        ...
        for topk_entry in topk_eos.keys():
            ...
            while curr_node != self.sos:
                ...
                (*_, curr_node, curr_idx) = all_node_topk_rouge_dict[curr_node][
                    curr_length
                ][curr_idx]
                curr_length -= 1
            path.reverse()
    ...
    def _extract_random_gain_paths(
        self, all_node_topk_rouge_dict, min_length=0, max_length=float("inf"), k=1
    ):
        topk_eos = heapdict()
        for eos in self.eos_list:
            for length, topk_dict in all_node_topk_rouge_dict[eos].items():
            ...
            for parent, logprob in self.reverse_edges[node].items():
                ...
                for parent_length, parent_topk_paths in parent_rouge_dict.items():
                    for parent_idx, parent_entry in parent_topk_paths.items():
                        ...
                        new_length = parent_length + 1
                        curr_word_prefix_count = 1 + (
                            self.nodes[parent]["text"] == curr_word
                        )
                        curr_node, curr_idx, curr_length = (
                            parent,
                            parent_idx,
                            parent_length,
                        )
                        while curr_node != self.sos:
                            (*_, curr_node, curr_idx) = all_node_rouge_dict[curr_node][
                                curr_length
                            ][curr_idx]
                            curr_length -= 1
                            curr_word_prefix_count += (
                                self.nodes[curr_node]["text"] == curr_word
                            )

                        curr_exp_match = exp_word_match.get(
                            (curr_word, curr_word_prefix_count), 0.0
                        )
                ...
                for parent_length, parent_topk_paths in parent_rouge_dict.items():
                    for parent_idx, parent_entry in parent_topk_paths.items():
                        ...
                        while len(ngram) < N and traversal_node != self.sos:
                            traversal_node, traversal_idx = all_node_rouge_dict[
                                traversal_node
                            ][traversal_length][parent_idx][-2:]
            ...
            for parent, logprob in self.reverse_edges[node].items():
                ...
                for parent_length, parent_topk_paths in parent_rouge_dict.items():
                    for parent_idx, parent_entry in parent_topk_paths.items():
                        ...
                        new_match = parent_match + curr_exp_match
                        # new_rouge = 2 * new_match / (new_length + mean_length)
                        new_log_rouge = np.log(new_match) - np.log(
                            new_length + mean_length
                        )
    ...
    def get_path_lprob(self, path):
        ...
        next(piter2)
        return sum(
            self.edges[curr_node][next_node]
...
class FastLattice(Lattice):
    ...
    def get_1gram_dict(
        ...
        def dfs_helper(node, length_to_here=0):
            ...
            for parent_node, edge_lprob in self.reverse_edges[node].items():
                parent_lprobs, parent_counts = dfs_helper(
                    parent_node, length_to_here=length_to_here + 1
                )
    ...
    def get_top_rouge1_path(
        ...
        def dfs_helper(node):
            ...
            curr_scores = torch.full(
                (max_len - min_len, len(parent_results) * lattice_topk), -1e6
            )
            for i, (parent_scores, par_min_len, par_max_len, parent_lprob) in enumerate(
                parent_results
            ):
                start_idx, end_idx = par_min_len - min_len, par_max_len - min_len
                curr_scores[
                    start_idx:end_idx
                ] = parent_scores + curr_exp_match * np.exp(parent_lprob)
