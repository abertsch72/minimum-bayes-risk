...
def pairwise_similarity_bertscore(topk_hypos, bert_model, bert_tokenizer):
    ...
    pairwise_token_sim = torch.tensordot(topk_hidden_states, topk_hidden_states, dims=([-1], [-1]))
    # (batch, max_seq_len, batch, max_seq_len)
    full_mask = attn_mask.unsqueeze(0).unsqueeze(0) * attn_mask.unsqueeze(-1).unsqueeze(-1)
...
for file in tqdm(result_files):
    ...
    if output.output is not None: # beam/baseline search
        ...
        # ]
        pairwise_sim = pairwise_similarity_bertscore(output.output, bert_model, bert_tokenizer)
        ...
        hypo_rouges = [full_rouge_scorer.score(output.reference, p) for p in output.output]
...
print(f"oracle rouge-1: mean = {np.mean(oracle_r1_scores)}, stddev = {np.std(oracle_r1_scores)}")
print(f"oracle rouge-2: mean = {np.mean(oracle_r2_scores)}, stddev = {np.std(oracle_r2_scores)}")
print(f"oracle rouge-l: mean = {np.mean(oracle_rl_scores)}, stddev = {np.std(oracle_rl_scores)}")
