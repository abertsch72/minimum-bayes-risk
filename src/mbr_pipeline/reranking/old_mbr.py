...
def main():
    ...
    rerank_rouge_scorer = rouge_scorer.RougeScorer(rerank_metrics, use_stemmer=True)
    ...
    get_ngram_dict_method_name = (
        f"get_{order}gram_dict{'_count_aware' if args.count_aware else ''}"
    )
    get_top_path_method_name = (
        f"get_top_rouge{order}_path{'_count_aware' if args.count_aware else''}"
    )
    ...
    for file in tqdm(result_files):
        ...
        length_dist_unnorm = (
            np.exp([length_dict[n][0] for n in lengths]) / lengths**args.length_alpha
        )
        ...
        ngram_dict, all_node_ngram_dict = get_ngram_dict_fn(
            ...
            allowed_deviation=args.length_deviation,
        )

        match_unweighted = {
            word: count / total_num_paths for word, (_, count) in ngram_dict.items()
        }
        match_weighted = {
            word: np.exp(lprob) for word, (lprob, _) in ngram_dict.items()
        }
        ...
        expected_match = (
            match_unweighted if (args.uniform or args.match_uniform) else match_weighted
        )
        ...
        topk_hypos = [
            ...
            for token_ids in topk_token_ids
        ]

        sim_matrix = pairwise_similarity(
            topk_hypos, rerank_rouge_scorer, rerank_metrics
        )
        max_idx = np.argmax(np.sum(sim_matrix, axis=-1))

        oracle_topk_rouges = [
            full_rouge_scorer.score(output.reference, hypo) for hypo in topk_hypos
        ]
        oracle_idx = max(
            range(len(topk_rouges)),
            key=lambda i: oracle_topk_rouges[i]["rouge2"].fmeasure,
        )
    ...
    print(
        "Average rouge-1 using MBR:",
        sum(data["rouge1"] for data in log_json) / len(log_json),
    )
    print(
        "Average rouge-2 using MBR:",
        sum(data["rouge2"] for data in log_json) / len(log_json),
    )
    print(
        "Average rouge-L using MBR:",
        sum(data["rougeL"] for data in log_json) / len(log_json),
    )
    print(
        "Oracle rouge-1 using MBR:",
        sum(data["oracle_rouge1"] for data in log_json) / len(log_json),
    )
    print(
        "Oracle rouge-2 using MBR:",
        sum(data["oracle_rouge2"] for data in log_json) / len(log_json),
    )
    print(
        "Oracle rouge-L using MBR:",
        sum(data["oracle_rougeL"] for data in log_json) / len(log_json),
    )
    ...
    wandb.log(
        {
            ...
            "oracle_rouge1": sum(data["oracle_rouge1"] for data in log_json)
            / len(log_json),
            "oracle_rouge2": sum(data["oracle_rouge2"] for data in log_json)
            / len(log_json),
            "oracle_rougeL": sum(data["oracle_rougeL"] for data in log_json)
            / len(log_json),
