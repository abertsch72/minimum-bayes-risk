...
def rescore_bartscore(topk_hypos, probs, bartscore=None):
    ...
    return bartscore.multi_ref_score(
        srcs=topk_hypos, 
        agg="mean",
        hypos=[[h for j, h in enumerate(topk_hypos) if j != i]
...
class Scorer(object):
    def __init__(self, metrics):
        self.rouge_metrics = [metric for metric in metrics if metric.startswith("rouge")]
    ...
    def score(self, gold, hypos) -> List[Score]:
        ...
        if self.bartscore is not None:
            bart_scores = self.bartscore.score(
                ...
                tgts=hypos, batch_size=min(len(hypos), 64)
            )
            for i in range(len(hypos)):
