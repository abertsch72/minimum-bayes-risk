...
from src.mbr_pipeline.list_gen.api_sample import OPENAI_MODELS, openai_listgen
...
def pipeline(args: Args):
    ...
    if args.rerank.rerank_metric is not None:
        reranker = Reranker(
            ...
            length_penalty=args.rerank.length_penalty,
        )
        ...
        if args.rerank.length_corrected:
            rerank_metric += f"_lencorr-{args.rerank.length_corrected}-{args.rerank.length_penalty}"
        ...
        if args.rerank.evidence_set_file is not None:
            ...
            if args.rerank.num_evidence is not None:
                ...
                filter_type = f"_nevid{args.rerank.num_evidence}"
            ...
            if args.rerank.num_hypo is not None:
                rerank_metric += f"_nhypo{args.rerank.num_hypo}"

        # all_evidence = []
        # for ev_idx, line in enumerate(evidence_outputs):
        #     for h in line['hypos']:
        #         all_evidence.append(h.replace("\n", " ").replace("\u2028", " "))
        # with open("evidence.en","w+") as f:
        #     f.write("\n".join(all_evidence))

        # all_hypos = []
        # for hy_idx, line in enumerate(sampling_outputs):
        #     all_hypos.append("NC=30")
        #     for h in line['hypos'][:30]:
        #         all_hypos.append(h.replace("\n", " ").replace("\u2028", " "))
        # with open("hypothesis.en","w+") as f:
        #     f.write("\n".join(all_hypos))
        ...
        if is_openai:
            model_length_penalty = 1.0
        else:
            gen_config = GenerationConfig.from_pretrained(args.pipeline.hf_model_name)
            model_length_penalty = getattr(gen_config, "length_penalty", 1.0)
        ...
        for idx, line in enumerate(tqdm(sampling_outputs)):
            ...
            if args.rerank.num_hypo is not None:
                ...
                rerank_line = deepcopy(line)
                rerank_line['hypos'] = rerank_line['hypos'][:args.rerank.num_hypo]
                rerank_line['lprobs'] = rerank_line['lprobs'][:args.rerank.num_hypo]


            # cross_scores_key = "cross_scores_bleu_logprobs_lencorr-mean-1.0temp-1.0_/home/alexx/lattice-search/test-outputs/wmt_ro_en/facebook-mbart-large-50-many-to-many-mmt/50/temp-temp=1.0top_p=1.0epsilon_cutoff=0.0.jsonl_nevid50_nhypo50"
            cross_scores_key = f"cross_scores_{rerank_metric}"

            cross_scores = line.get(cross_scores_key)
            if cross_scores is not None:
                cross_scores = np.array(cross_scores)

            # all_hypos.append(f"NC={len(rerank_line['hypos'])}")
            # all_hypos.extend(rerank_line['hypos'])
            # all_evidence.extend(evidence_set['hypos'])
            ...
            scores = reranker.rerank(
                rerank_line, evidence_set, tokenizer=tokenizer, lattice=lattice,
                model_length_penalty=model_length_penalty, cross_scores=cross_scores
            )

            if isinstance(scores, tuple):
                scores, cross_scores = scores
            line[scores_key] = scores
            # line['hypos'] = rerank_line['hypos']
            # line['lprobs'] = rerank_line['lprobs']
