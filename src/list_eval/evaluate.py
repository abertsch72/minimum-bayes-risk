...
class Metrics:
    ...
    def evaluate(self, item):
        ...
        scores = None
        re_scored = False
        metrics_to_compute = self.eval_metrics
        if cached_scores:
            cached_keys = cached_scores[0]['score_dict'].keys()
            # if set(cached_keys) == set(self.eval_metrics):
            scores = [Score(d["score_dict"], d["is_pct"]) for d in cached_scores]
            metrics_to_compute = set(metrics_to_compute) - set(cached_keys)
        # metrics_to_compute.add("lengthratio")
        if metrics_to_compute:
            if len(metrics_to_compute) < len(self.eval_metrics):
                self.scorer = Scorer(metrics_to_compute)
            re_scored = True
            new_scores = self.scorer.score(gold, hypos)
            if scores is not None:
                scores = [Score({**s.score_dict, **ns.score_dict}, s.is_pct) for s, ns in zip(scores, new_scores)]
            else:
                scores = new_scores
        ...
        for rerank_type in rerank_types:
            if "mergeevidence" in rerank_type:
                continue
            ...
            if (
                ...
                if max_rerank_idx >= len(scores):
                    print(rerank_type, "exceeded max length")
