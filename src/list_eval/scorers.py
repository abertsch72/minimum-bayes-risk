...
# Copied with slight modification from rouge-score code
...
# Overwrite rouge-score tokenizer
...
def rescore_bertscore(topk_hypos, probs, bertscore=None, evidence_set=None):
    ...
    if probs is None:
        probs = [1/len(evidence_set)] * len(evidence_set)
    predictions = []
    references = []
    ...
    f1scores = np.array(score['f1'])
    n_hypos = len(topk_hypos)
    n_evid = len(evidence_set)
    probs = np.array(probs)
    f1scores = f1scores.reshape((n_hypos, n_evid))
    avg_scores = f1scores @ probs.reshape((n_evid, 1))
    return avg_scores.reshape(-1).tolist(), f1scores
...
def rescore_bleurt(topk_hypos, probs, bleurt=None, evidence_set=None):
    ...
    n_hypos = len(topk_hypos)
    n_evid = len(evidence_set)
    scores = np.array(scores).reshape(n_hypos, n_evid)
...
def rescore_rouge(topk_hypos, probs, rouge, eps=1e-4, evidence_set=None):
    ...
    return (sim_matrix @ probs[:, None])[:, 0].tolist(), sim_matrix
...
def rescore_bleu(topk_hypos, probs, max_ngram_order=4, evidence_set=None):
    ...
    k, m = len(topk_hypos), len(evidence_set)
    sim_matrix = np.zeros((k, m)) - 1
    for i in range(k):
        for j in range(m):
            if sim_matrix[i, j] != -1:
    ...
    if probs is None:
        return sim_matrix.mean(axis=-1).tolist(), sim_matrix
    return (sim_matrix @ probs[:, None])[:, 0].tolist(), sim_matrix
...
class Scorer(object):
    def __init__(self, metrics):
        ...
        self.length_ratio_scorer = None
        if "lengthratio" in metrics:
            self._stemmer = porter.PorterStemmer()
            self.length_ratio_scorer = lambda hypo, ref: len(hypo) / len(ref)
    ...
    def score(self, gold, hypos) -> List[Score]:
        ...
        if self.bleu_scorer is not None:
            for i, hypo in enumerate(hypos):
                ...
                scores[i]["bleu"] = bleu
        if self.length_ratio_scorer is not None:
            ref_tokens = tokenize.tokenize(gold, self._stemmer)
            for i, hypo in enumerate(hypos):
                hypo_tokens = tokenize.tokenize(hypo, self._stemmer)
                length_ratio = self.length_ratio_scorer(hypo_tokens, ref_tokens)
                scores[i]["lengthratio"] = length_ratio
