...
class Reranker:
    ...
    def rerank(
        self, item: dict, evidence_set: List[dict] = None, tokenizer=None, lattice=None, model_length_penalty=1,
        cross_scores=None
    ) -> List[int]:
        ...
        if lprobs is not None and self.length_corrected:
            ...
            lprobs_uncorrected = lprobs * (lengths**model_length_penalty)
            if self.length_corrected == 'mean':
                lprobs_corrected = lprobs_uncorrected / (lengths**self.length_penalty)
            elif self.length_corrected == 'google':
                factor = ((5+lengths) / 6)**self.length_penalty
                lprobs_corrected = lprobs_uncorrected / factor
            elif self.length_corrected == 'reward':
                lprobs_corrected = lprobs_uncorrected + lengths * self.length_penalty
            else:
                raise Exception("Invalid length correction:", self.length_corrected)
            ### experimental code
        ...
        if cross_scores is not None:
            rerank_scores = (cross_scores @ probs[:, None])[:, 0].tolist()
            rerank_scores = (rerank_scores, cross_scores)
        else:
            rerank_scores = self.rerank_fn(hypos, probs, evidence_set=evidence_hypos)
